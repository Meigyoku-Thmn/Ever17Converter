...
whole body.`;
   `Something...like a hard shell, began to 
...
and soon covered the entire shell.`;
   ...
   `I was able to glimpse what lay beyond the shed 
husk...it was dazzling..`;
   `Particles of light, whiter than white, filled 
...
text(() => {
   ...
   `If you stare at something for a long time, it 
...
on me.`;
   `And not thinking any more about it...I shook 
...
you got the two of us mixed up..."`;
   ...
   Coco(T3A016)`"We need to get ready or we'll be late for 
school..."`;
...
showTextbox();
text(() => {
   `When Coco and I made it to the kiosk, 
...
showTextbox();
text(() => {
   Kid(T3A031)`"I'm not sure why, but you look like you're 
having fun."`;
...
showTextbox();
text(() => {
   Coco(T3A032)`"Yeah, you look like might even be enjoying 
your life here."`;
...
text(() => {
   ...
   Coco(T3A034)`"Takepyon, you really do look like you work 
here."`;
   Takeshi`"Hot dogs, get your hotdogs! They're on the 
house!"`;
   Coco(T3A035)`"You don't think, that maybe you could have 
...
showTextbox();
text(() => {
   You(T3A036)`"No, Coco, I don't think he was a hotdog man, 
I think he was..."`;
...
so much that first night.`;
   `For no reason, Coco was playing with Pipi's 
tail.`;
...
text(() => {
   ...
   `The girl and I pointed at each 
other and froze.`;
   Pipi(PIPI_00)`"Woof!"`;
...
around her feet.`;
   ...
   Coco(C1T027)`"This little guy's name is Pipi, and my name's 
Coco....Try not to mix them up, okay?"`;
   Takeshi`"But...what are you doing here?"`;
   Coco(C1T028)`"Well, what do you expect me to say 
to that...?"`;
   Coco(C1T029)`"I was trying to go to one of the lower 
...
elevator. Try not to mix them up."`;
   ...
   Coco(C1T032)`"Anyways, I don't know why, but all of a 
...
my hand out to Coco.`;
   ...
   `Coco held up Pipi to me, stretching as far as 
...
text(() => {
   ...
   `Coco jumped over and over, reaching out for 
my hand.`;
   `But... Either she was too short, or just had a 
...
illusion generated by RSD...`;
   ...
   `I thought about what we'd talked about the day 
before......`;
...
exercise."`;
   Sora(C2T016)`"My question was whether there is any way of 
...
text(() => {
   ...
   Sora(C2T022)`"You sounded so full of confidence when 
...
showTextbox();
text(() => {
   `Her existence disappeared when I close my 
eyes...`;
...
showTextbox();
text(() => {
   Tsugumi(T1C121)`"Hmmm...? I guess you're not as dumb as you 
look."`;
...
text(() => {
   ...
   Tsugumi(T1C126)`"No, not just you, but everyone else as well, 
all four of them."`;
...
showTextbox();
text(() => {
   Tsugumi(T1C129)`"Yeah. I suppose I could die in a place like 
this."`;
...
on her face as she said those words.`;
   `I was thinking this and looking at her. I 
...
showTextbox();
text(() => {
   `I'd bumped into You and Coco in front of the 
...
text(() => {
   ...
   Coco(C2T038)`"These are from when Nakkyu was a litte bitty 
baby!"`;
...
sneaked into school that one night..."`;
   You(C2T051)`"And of course the graduation 
...
showTextbox();
text(() => {
   You(C2T054)`"A history album.... Well, I've never called 
it that before..."`;
...
showTextbox();
text(() => {
   You(C2T060)`"I wonder what everyone is doing right about 
now...?"`;
...
herself get caught up in things."`;
   ...
   You(C2T078)`"I've been living with her for 18 years... 
...
showTextbox();
text(() => {
   `Coming out of my reveries, I noticed the 
...
text(() => {
   ...
   `Coco was hugging him so hard he seemed about 
to rupture.`;
...
showTextbox();
text(() => {
   Coco(C3A003)`"Hey, did something just happen? There was an 
awful sound just now."`;
   Sora(C3A004)`"One of the shafts that supports this floor 
has started to bend out of alignment."`;
   Sora(C3A005)`"It appears as if the ocean currents may have 
caused it."`;
...
showTextbox();
text(() => {
   Tsugumi(T3A077)`"Well, if we're going to fix something, 
...
shoes.`;
   `From the back of the room, we could hear a 
...
showTextbox();
text(() => {
   Tsugumi(T3A116)`"The pipe is bent a little bit...I'm going to 
...
showTextbox();
text(() => {
   Tsugumi(T3A118)`"On the other side of this...there should be 
...
showTextbox();
text(() => {
   `In this way, the three of us started working 
in unison.`;
...
showTextbox();
text(() => {
   Tsugumi(T3A125)`"Takeshi? Don't just stand there, on to the 
next pipe."`;
   Takeshi`"Huh? Oh, okay."`;
...
text(() => {
   ...
   Tsugumi(T3A127)`"After that I want you to turn the red 
valve..."`;
...
showTextbox();
text(() => {
   Tsugumi(T3A130)`"Takeshi, move the valves in the order I told 
you. Now."`;
   Takeshi`"......"`;
...
showTextbox();
text(() => {
   `I set myself upon the blue valve in front of 
me.`;
   `I was supposed to loosen this...`;
   `I'm supposed to turn this counter-clockwise, 
yeah...`;
...
showTextbox();
text(() => {
   Tsugumi(T3A138)`"Takeshi, after I weld this on, I want you to 
...
showTextbox();
text(() => {
   `I quickly turned the valves in the opposite 
direction.`;
...
showTextbox();
text(() => {
   Tsugumi(T3A140)`"Coco, this pipe's done. Go on to the next 
one."`;
...
text(() => {
   ...
   Tsugumi(C3A039)`"The water that was in the suction pipe is 
...
corners flashing.`;
   `I landed in the water - I'd thrown myself out 
...
text(() => {
   ...
   `Stung by Tsugumi's cold words, Coco 
...
text(() => {
   ...
   Tsugumi(T3A156)`"At this rate, this room's partitions won't 
last."`;
...
showTextbox();
text(() => {
   Tsugumi(T3A160)`"There's nothing you can do, right? I'm stuck 
here."`;
   Tsugumi(T3A161)`"And with this injury, you can't help me 
anyway——"`;
   Takeshi`"Shut up!!"`;
   `I roared out.`;
...
legs to get closer to the intercom.`;
   Coco(C3A058)`"It's no use.... It's broken! I pushed the 
...
text(() => {
   ...
   Tsugumi(T3A166)`"It doesn't mean anything... just leave. I 
don't want you here."`;
...
showTextbox();
text(() => {
   Tsugumi(T3A168)`"Oh, really...heh, heh. Well, that's reason 
enough I suppose. "`;
...
showTextbox();
text(() => {
   Tsugumi(T3A176)`"There you go again, saying 'It'll be 
...
showTextbox();
text(() => {
   Sora(C3A077)`"Well, this is just a bio scan, so I can't 
tell for certain, but..."`;
   Sora(C3A078)`"Her femur is fractured. And it looks like it 
might be compound."`;
   Sora(C3A079)`"That and the artery is severed. She'll need 
sutures."`;
...
showTextbox();
text(() => {
   You(C3A080)`"I've stitched up my finger before. It was 
...
text(() => {
   ...
   Sora(C3A083)`"I'll get it ready. The laser scalpel, 
...
tools.`;
   `You flipped up Tsugumi's skirt, and fearlessly 
...
text(() => {
   ...
   `Her body trembling, she asked me this over 
and over.`;
   `I thought maybe she had been trying not to 
cry until that moment.`;
   `Coco, head pressed against my chest started 
crying.`;
...
showTextbox();
text(() => {
   `All these voices jumped back and forth in my 
...
showTextbox();
text(() => {
   Sara(C3A114)`"How can you talk about that kind of dream so 
calmly?"`;
   Kid`"Well it's not as if I WANTED to see it..."`;
   Sara(C3A115)`"Hmmm..."`;
   Sara(C3A116)`"So that's why you decided to run into here 
all of a sudden?"`;
...
showTextbox();
text(() => {
   Sara(C3A117)`"Well, I guess it's possible that you'd start 
...
showTextbox();
text(() => {
   Sara(C3A118)`"Of course you were! Like a hysteric three 
year old."`;
...
text(() => {
   ...
   Kid`"?"`;
   Sara(C3A121)`"You just said 'The warehouse started 
flooding,' right?"`;
   Kid`"Yeah."`;
   Sara(C3A122)`"That actually happened."`;
   Kid`"...What?"`;
   Sara(C3A123)`"When you were sleeping, a pipe burst in the 
...
showTextbox();
text(() => {
   Sara(C3A124)`"But, don't worry, okay? It got fixed without 
any hitches."`;
...
showTextbox();
text(() => {
   Sara(C3A128)`"You didn't have an out-of-body experience, 
did you?"`;
...
text(() => {
   `Moron.`;
   Sara(C3A130)`"Even if you could look up somebody's skirt, 
...
text(() => {
   ...
   Sara(C3A132)`"I was really worried last night that you 
...
text(() => {
   `—The changing room.`;
   `It was a rectangular room...and stuck to the 
...
showTextbox();
text(() => {
   `I jumped about three feet in the air, bending 
...
showTextbox();
text(() => {
   Sara(C3A146)`"'Tis but a simple thing, to conceal the 
...
showTextbox();
text(() => {
   Sara(C3A147)`"Well, you didn't say anything, you just took 
off."`;
   Kid`"So you went and followed me?"`;
   Sara(C3A148)`"Yeah."`;
   `Sara had grabbed on to her tied-up hair with 
...
showTextbox();
text(() => {
   `I pulled Sara by the arm over to stand in 
front of the mirror.`;
...
mirror's surface.`;
   `The left and right side of her face was 
...
mistaking it was her.`;
   ...
   Sara(C3A153)`"Well, if it isn't your face, whose face is 
it?"`;
   Kid`"Hmmm..."`;
   Kid`"I guess you're right..."`;
   `I knew the answer, but I still had needed to 
check.`;
   `This kid was...me.`;
...
didn't know..."`;
   Sara(C3A157)`"Hmmm..."`;
   Sara(C3A158)`"But that shouldn't be enough to make you 
foam at the mouth, don't you think?"`;
   Sara(C3A159)`"I mean if you looked into the mirror and 
suddenly were hopelessly ugly, even I would 
faint, but..."`;
   Sara(C3A160)`"Your face...isn't that bad..."`;
   Sara(C3A161)`"I mean you'd think that you'd be relieved and 
think, 'You know I've got a pretty nice 
looking face.'"`;
   `Sara glanced at the mirror, and took a good 
look at my face. `;
   Sara(C3A162)`"I mean, if I had to say one way or the 
other, I like your face."`;
   `And, so saying, Sara pointed the tip of her 
hair at me.`;
...
paralyzing my ability to think.`;
   ...
   Sara(C3A169)`"Or maybe, I guess you could say I'm giving 
myself a compliment..."`;
