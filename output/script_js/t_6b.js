...
showTextbox();
text(() => {
   You(T6B000)`"I'm going to scan her, so put her up on the 
table!"`;
...
showTextbox();
text(() => {
   You(T6B006)`"If you don't tell us, we don't know what's 
going on..."`;
   Sora`"........."`;
...
text(() => {
   ...
   Sora(T6B014)`"It is a powerful virus with a very high 
mortality rate."`;
...
showTextbox();
text(() => {
   Kid(T6B023)`"If we don't do anything, Coco's...going to 
die!"`;
...
showTextbox();
text(() => {
   Sora(T6B026)`"...If we take emergency measures, it is 
possible."`;
...
showTextbox();
text(() => {
   You(T6B028)`"Whatever it takes! If we don't hurry, 
then...!"`;
...
showTextbox();
text(() => {
   You(T6B029)`"Please Sora...you're the only one who can 
help us."`;
...
showTextbox();
text(() => {
   Sora(T6B031)`"You'll need serum. A serum to lessen the 
symptoms."`;
...
text(() => {
   ...
   You(T6B047)`"Eh!?"`;
   Sora(T6B048)`"That thing that Pipi has in his mouth! 
That's it!"`;
   Takeshi`"What!?"`;
...
liquid inside of it.`;
   ...
   Sora(T6B050)`"Break off the tip, and inject the contents 
into Coco!"`;
   Takeshi`"O-okay!!"`;
...
text(() => {
   ...
   Sora(T6B052)`"Her condition has improved somewhat. The 
...
showTextbox();
text(() => {
   Sora(T6B055)`"Alright. I will tell you everything I know 
...
the only noise to echo through the room.`;
   ...
   Sora(T6B057)`"TB was developed at a research facility 
...
text(() => {
   ...
   Sora(T6B059)`"Yes..."`;
   Sora(T6B060)`"There's an installation called IBF, right 
below us."`;
...
text(() => {
   `Tsugumi frowned.`;
   Tsugumi(T6B063)`"Well, it looks like this AI's programming CAN 
change after all. If the situation gets 
critical enough...we can actually get 
information we need..."`;
   Tsugumi(T6B064)`"I guess it isn't in your programming to keep 
...
institute in order to study them.${wait(30)}`;
...
text(() => {
   ...
   Sora(T6B069)`"Only a select number of people know about it 
...
showTextbox();
text(() => {
   Sora(T6B070)`"IBF and LeMU are controlled by separate 
computer systems..."`;
   Sora(T6B071)`"Even I am only given limited information."`;
   Sora(T6B072)`"I only became sure of IBF's existence after 
connecting together fragments of information 
from this accident...."`;
   Takeshi`"Oh..."`;
   You(T6B073)`"Anyway, there is a strange and deadly virus 
called TB they're doing research about at 
some research center under LeMU."`;
   You(T6B074)`"That is probably in order to find a cure for 
TB, right?"`;
...
superior to this one."`;
   Sora(T6B077)`"At least from the information I can gather 
...
should increase..."`;
   ...
   Sora(T6B081)`"We might even encounter more of the serum 
...
showTextbox();
text(() => {
   Kid(T6B085)`"It doesn't make sense that she'd come down 
...
showTextbox();
text(() => {
   Tsugumi(T6B088)`"Even if we want to help Coco, there isn't any 
equipment here to do that."`;
   Tsugumi(T6B089)`"The only lead we have for treating her is 
...
showTextbox();
text(() => {
   Tsugumi(T6B091)`"All we can do is trust what Sora tells us 
for now."`;
...
showTextbox();
text(() => {
   Tsugumi(T6B092)`"She isn't good enough at lying to make up 
...
showTextbox();
text(() => {
   Sora(T6B106)`"It is unknown to me. It was only given to 
...
showTextbox();
text(() => {
   Sora(T6B108)`"Accessing the security block for LeMMIH's 
system..."`;
...
showTextbox();
text(() => {
   You(T6B119)`"If it won't open any other way, I'm going to 
break it down!"`;
...
text(() => {
   ...
   Tsugumi(T6B125)`"I'm wracking my brains thinking about the 
same thing."`;
...
showTextbox();
text(() => {
   Tsugumi(T6B126)`"Do you know what kind of process this door 
goes through when it opens?"`;
   Sora(T6B127)`"Okay, let me see..."`;
   Sora(T6B128)`"After the correct keys are input, a lever 
embedded inside the hatch should start to 
raise."`;
   Sora(T6B129)`"As that lever turns, the lock on the door is 
released."`;
...
showTextbox();
text(() => {
   Sora(T6B130)`"However the lever is nothing more than an 
...
showTextbox();
text(() => {
   Tsugumi(T6B131)`"Yeah...I suppose they designed it so that you 
...
showTextbox();
text(() => {
   Tsugumi(T6B133)`"If we can get that lever off, maybe we can 
...
showTextbox();
text(() => {
   `${voice(T6B136A)}"Öffnen Sie ${waitVoice}${voice(T6B136B)}die Tür von HIMMEL"`;
...
searched for me by my voice.`;
   Researcher(T6B143)`"Doesn't seem like you're...here to rescue 
me..."`;
   `He grabbed onto me with shaking hands.`;
   `They were covered in blood...`;
   Researcher(T6B144)`"So there were still people in LeMU...and 
...
showTextbox();
text(() => {
   Tsugumi(T6B149)`"Takeshi, cut it out, you're shaking him too 
hard."`;
   Tsugumi(T6B150)`"He's unconscious. Just leave him alone for 
now."`;
   Takeshi`"O-okay..."`;
...
showTextbox();
text(() => {
   Tsugumi(T6B162)`"What's the matter, Takeshi? You aren't 
...
showTextbox();
text(() => {
   Tsugumi(T6B165)`"Saying something like that at a time like 
this...you really are a moron."`;
   Sora(T6B166)`"Takeshi, it will only take one hour. I'm sure 
everyone can wait."`;
...
the open space available.`;
   ...
   Sora(T6B168)`"Once the compression is completed, the door 
...
but spoke to us from in front of the entrance.`;
   Sora(T6B169)`"I will expedite the compression process. It 
...
arrival, the lights went on in the room.`;
   ...
   You(T6B180)`"Umm, I think they just turned on by 
themselves..."`;
   Tsugumi(T6B181)`"The sensors reacted, and the computer 
...
Can you help us? Please!"`;
   Researcher(T6B189)`"Oxygen treatment...yeah, you're looking for 
the pods."`;
   Researcher(T6B190)`"There should be some alloy capsules, with a 
mat set inside of them."`;
   Researcher(T6B191)`"They look like a bunch of cylinders stuck to 
...
hand, as he wiped his mouth.`;
   Researcher(T6B193)`"Just take a look at the manual...they should 
be...easy to operate."`;
...
cryogenic suspension..."`;
   ...
   You(T6B223)`"We don't really know is going to happen to 
us."`;
