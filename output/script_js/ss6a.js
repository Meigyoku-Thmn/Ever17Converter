...
text(() => {
   `${voice(S6A000)}
${noWait}`;
   Append(S6A001)`      ${nextPage(0)}Moon sprite shouldering a long bow.${nextPage(1)}${waitVoice}
${noWait}`;
   `${voice(S6A002)}            ${nextPage(0)}Waiting inside a dream.${nextPage(1)}${waitVoice}
${noWait}`;
   `${voice(S6A009)}         ${nextPage(0)}Tonight a story by moonlight.${nextPage(1)}${waitVoice}
${noWait}`;
   `${voice(S6A004)}         ${nextPage(0)}Hoping the wait will be short.${nextPage(1)}${waitVoice}
${noWait}`;
   `${voice(S6A005)}           ${nextPage(0)}Sleeping curled and snug.${nextPage(1)}${waitVoice}
${noWait}`;
...
text(() => {
   `${voice(S6A006)}
${noWait}`;
   Append(S6A007)`    ${nextPage(0)}Water sprite lifting a pair of paddles.${nextPage(1)}${waitVoice}
${noWait}`;
   `${voice(S6A008)}            ${nextPage(0)}Waiting inside a dream.${nextPage(1)}${waitVoice}
${noWait}`;
   `          ${nextPage(0)}Tonight the whirlpool demon, 
${noWait}`;
   `${voice(S6A009)}               waiting for a boat${nextPage(1)}${waitVoice}
${noWait}`;
   `${voice(S6A010)}         ${nextPage(0)}Hoping the wait will be short.${nextPage(1)}${waitVoice}
${noWait}`;
   `${voice(S6A011)}          ${nextPage(0)}Rock, rock, rocked to sleep.${nextPage(1)}${waitVoice}
${noWait}`;
...
manipulator arm above the examination table.`;
   Sora(S6A018)`"This was embedded in your thumb, so 
I extracted it."`;
...
showTextbox();
text(() => {
   Sora(S6A020)`"However, no good can come of leaving a 
...
text(() => {
   ...
   Sora(S6A022)`"Kid, do you recall seeing this fragment 
before?"`;
   Kid`"No! How could I?"`;
...
pendant.`;
   ...
   Sora(S6A026)`"This hologram was designed so that it only 
can be seen when viewed with light of a 
certain wavelength."`;
   Sora(S6A027)`"Right now I am using LeMMIH to shine the 
proper light on it."`;
   Sora(S6A028)`"I could not confirm the date of manufacture 
of this hologram, but it is undoubtedly very 
old."`;
   Sora(S6A029)`"Its makeup is fundamentally different from 
the ones being sold at the LeMU's kiosks."`;
   Sora(S6A030)`"In the past, holograms were recorded using an 
interference pattern made from an object beam 
and a reference beam."`;
   Sora(S6A031)`"Put simply, the object beam is the light 
reflected off of the object."`;
   Sora(S6A032)`"The reference beam is the light that 
...
to think.`;
   ...
   Sora(S6A035)`"Light, which is the same wavelength as the 
reference beam used to record the image, 
must be shined upon it."`;
   Sora(S6A036)`"So, to sum up, it is like this."`;
   Sora(S6A037)`"The image on this fragmented hologram was 
recorded using light that was a certain 
wavelength."`;
   Sora(S6A038)`"And in order to see the image you must shine 
light of the same wavelength on it."`;
   Sora(S6A039)`"It also cannot be seen under fluorescent 
light."`;
   Sora(S6A040)`"You must use light generated by 
...
light.`;
   `She went on to say that if a person were able 
...
showTextbox();
text(() => {
   Sora(S6A042)`"You would not be able to see this image 
unless you can see infrared light."`;
   Sora(S6A043)`"The fact that you can see this image means 
...
showTextbox();
text(() => {
   Sora(S6A044)`"Yes. Both of you have a very unique ability 
...
text(() => {
   ...
   Sora(S6A046)`"The surface and shape of the fragment match 
...
showTextbox();
text(() => {
   Sora(S6A049)`"It is the same reason that you both have the 
ability to see infrared light."`;
   Sora(S6A050)`"It is probably because you both carry the 
same genes."`;
   Kid`"No..."`;
   Kid`"No...no...no..."`;
...
text(() => {
   ...
   Sora(S6A053)`"Is there any better way possible to recover 
your memory?"`;
   Kid`"No...no..."`;
...
showTextbox();
text(() => {
   Sora(S6A054)`"In either case, it is a fact and you should 
accept it."`;
   Sora(S6A055)`"While you two were asleep I performed a DNA 
analysis on both of you."`;
   Sora(S6A056)`"The results are indicated on the display."`;
   Sora(S6A057)`"The probability that you are brother and 
...
bullies in an open space.`;
   `It was Sara.`;
   `The kids were being cruel, jeering her and 
...
but everyone had shunned us.`;
   ...
   `Our caretakers at the orphanage where lived 
...
lone window.`;
   `The only things I remember are the Leiblich 
...
text(() => {
   ...
   Sara(S6A066)`"But come for sure. You come to get me for 
sure. Okay?"`;
...
showTextbox();
text(() => {
   `I was moved to another facility and, after a 
...
showTextbox();
text(() => {
   `Even now, Sara was on the other side 
of the door....`;
...
or I had dragged myself unconsciously.`;
   ...
   `Not much time had passed since I had 
passed out.`;
   `Laying on the bed, I looked up at the ceiling 
...
made sense, too.`;
   ...
   `Probably, even without my memory, in the back 
...
text(() => {
   ...
   Sara(S6A083)`"You are my brother, right? You are, aren't 
you?"`;
   Kid`"No, no. I'm..."`;
...
shrieking.`;
   `All around me the squeal of straining metal 
...
text(() => {
   ...
   You(S6A090)`"I'll explain later. For now just hurry over 
here."`;
   Kid`"W-what?"`;
...
showTextbox();
text(() => {
   `With those words of encouragement, You started 
running.`;
...
leading to an emergency corridor.`;
   ...
   You(S6A093)`"Takeshi is going to get Mayo. Tsugumi is 
...
text(() => {
   ...
   `I started running.`;
   You(S6A097)`"We were finally able to make contact with 
the surface and now the emergency stairs 
are accessible."`;
   You(S6A098)`"But if we don't hurry, they're 
going to flood."`;
...
text(() => {
   ...
   `You stood on the stairs, stunned and looking 
downward.`;
...
showTextbox();
text(() => {
   Takeshi(S6A107)`"Geez...Didn't you check to see that 
she was behind you?"`;
...
down the stairs I had just climbed.`;
   ...
   You(S6A115)`"It's impossible! It's already starting to 
...
showTextbox();
text(() => {
   `I went down the stairs and came out at Zweite 
...
probably wouldn't have made it to Dritte stock.`;
   ...
   `If she had gone all the way down to Dritte 
stock...`;
...
there and the flooding more serious.`;
   `If she were in Dritte stock, then I'd have to 
...
I peeked inside.`;
   `If Sara were there, I'd be able to see her 
body heat.`;
...
into the Lemurian Ruins, but Sara wasn't there.`;
   ...
   `The sound of the water grew louder and the 
...
text(() => {
   ...
   `This room also had a pocket of air near 
the ceiling.`;
