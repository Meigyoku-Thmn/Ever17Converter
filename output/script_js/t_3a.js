...
showTextbox();
text(() => {
   You(T3A002)`"What are you talking about? You're just 
lazy..."`;
...
showTextbox();
text(() => {
   You(T3A003)`"Then again, being all tense for so long just 
...
showTextbox();
text(() => {
   You(T3A004)`"Being in a situation like this, we really 
need to keep well rested."`;
   You(T3A005)`"'Cause when it comes down to it, if we can't 
...
text(() => {
   ...
   You(T3A007)`"Hey, since you're up, why don't you go wash 
your face?"`;
...
all."`;
   Kid(T3A011)`"Well, a lot's happened in the last couple 
...
showTextbox();
text(() => {
   Coco(T3A032)`"Yeah, you look like you might even be 
...
text(() => {
   ...
   Kid(T3A067)`"Hey, did something just happen? There was an 
...
has started to warp."`;
   ...
   Sora(T3A070)`"Yes, well, it wasn't in my calculations. It 
...
text(() => {
   ...
   Tsugumi(T3A077)`"Well, if we're going to fix something, we'll 
...
showTextbox();
text(() => {
   Kid(T3A079)`"She went to the Control Room when things were 
shaking earlier."`;
...
text(() => {
   ...
   Kid(T3A081)`"I'll go and tell her that you're going up 
above."`;
...
showTextbox();
text(() => {
   Sora(T3A082)`"If something comes up, use LeMMIH to contact 
me."`;
...
text(() => {
   ...
   Sora(T3A086)`"Actually, there was a routine maintenance on 
...
showTextbox();
text(() => {
   Sora(T3A087)`"It appears that it took some time for the 
...
sudden?"`;
   Sora(T3A096)`"Well.... Everything doesn't always go 
according to plan..."`;
...
showTextbox();
text(() => {
   Coco(T3A097)`"Is it because everyone was bouncing around so 
much?"`;
...
showTextbox();
text(() => {
   Coco(T3A098)`"Well, because everyone was making such a 
commotion."`;
...
showTextbox();
text(() => {
   Tsugumi(T3A101)`"But if we make some repairs, everything 
should be fine."`;
...
showTextbox();
text(() => {
   Sora(T3A106)`"I'm going to perform a scan to determine the 
status of the area."`;
...
text(() => {
   ...
   Sora(T3A111)`"Atmospheric pressure is normal. No traces of 
toxic gas detected. The temperature and 
humidity of the room are rising in slight 
increments."`;
   Sora(T3A112)`"An area of the pipe has ruptured, and there 
...
showTextbox();
text(() => {
   Tsugumi(T3A122)`"You know, asking meaningless questions is a 
bad habit of yours."`;
...
showTextbox();
text(() => {
   Sora(T3A123)`"I'm sorry everyone, It seems like You is 
...
showTextbox();
text(() => {
   Tsugumi(T3A135)`"Takeshi, go support that pipe, it looks like 
it's shaking."`;
...
showTextbox();
text(() => {
   Tsugumi(T3A136)`"Huh? That shouldn't be.... Did you turn the 
valve the right way?"`;
...
showTextbox();
text(() => {
   Tsugumi(T3A138)`"Takeshi, after I've finished attaching this, 
...
grimy.`;
   Tsugumi(T3A143)`"The drainage is starting to back-flow, be 
careful!"`;
...
legs to get closer to the intercom.`;
   Coco(T3A163)`"It's no use.... It's broken! I pushed the 
...
text(() => {
   ...
   Sora(T3A179)`"Bring her to the infirmary immediately, You 
is currently preparing to treat her."`;
   Takeshi`"Alright, I'm on my way! Now, which way do I 
go?"`;
   Sora(T3A180)`"Return back along this passage, go down the 
...
showTextbox();
text(() => {
   Sora(T3A185)`"Well, this is just a bio scan, 
...
showTextbox();
text(() => {
   Sora(T3A186)`"Her femur is fractured. And it looks like it 
might be compound."`;
   Sora(T3A187)`"That and the artery is severed. She'll need 
sutures."`;
...
showTextbox();
text(() => {
   You(T3A188)`"I've stitched up my finger before. It was 
...
text(() => {
   ...
   Sora(T3A191)`"I'll get it ready. The laser scalpel, 
...
showTextbox();
text(() => {
   Sora(T3A199)`"It required a total of 40 stitches 
and will take two months to heal completely."`;
   Sora(T3A200)`"And it will take her a few more months to be 
able to walk normally..."`;
   Sora(T3A201)`"Until a rescue team arrives at LeMU, and we 
...
showTextbox();
text(() => {
   You(T3A203)`"Oh...um, nothing, I'm just really tired...you 
know..."`;
   You(T3A204)`"I've never done an operation like that 
before..."`;
...
showTextbox();
text(() => {
   Sora(T3A205)`"By the way, it looks like the flooding 
stopped in the Zweite stock warehouse."`;
   Sora(T3A901)`"All pipelines connecting to the warehouse 
have been closed as an emergency measure and 
the whole area has been sealed off from 
Zweite. `;
   Sora(T3A902)`So, we don't have to worry about the 
