...
showTextbox();
text(() => {
   `We met at the chicken sandwich stand and ate 
as scheduled.`;
...
text(() => {
   ...
   Takeshi`"Whaaaat?"`;
   Kid(T3C001)`"Can't you fix anything besides chicken 
sandwiches?"`;
...
text(() => {
   ...
   `You was done eating, and she peered at the 
Kid's face.`;
   Kid(T3C004)`"Yeah..." `;
...
showTextbox();
text(() => {
   You(T3C005)`"You must have walked all over looking 
something else to eat. But it was no use 
after all, huh?"`;
   You(T3C006)`"The food washed into the corridors during the 
...
showTextbox();
text(() => {
   You(T3C007)`"But that's okay because we've more stuff to 
...
shoved it in his mouth.`;
   You(T3C009)`"To survive, it's crucial that you have 
...
text(() => {
   Takeshi`"Huh?"`;
   You(T3C011)`"Yeah, just a little hobby of mine. Sometimes 
I go into the mountains alone."`;
   You(T3C012)`"I carve my own path...sometimes I even get a 
bit lost..."`;
...
showTextbox();
text(() => {
   You(T3C013)`"Geez, Takeshi. Why do you have to get like 
that?"`;
...
showTextbox();
text(() => {
   You(T3C014)`"Nope. I hunt out hot springs that nobody 
knows about."`;
...
showTextbox();
text(() => {
   You(T3C015)`"Yeah, deep in the mountains, someplace no 
...
showTextbox();
text(() => {
   You(T3C016)`"No, no, no. Would you shut up about bears 
already!"`;
   Takeshi`"Heh, heh..."`;
...
showTextbox();
text(() => {
   You(T3C017)`"Oh, what might you be suggesting? Oh, heh, 
heh. "`;
...
text(() => {
   ...
   You(T3C019)`"But it was just a surface scratch and I only 
...
down...)`;
   `Just as I was thinking that, You avoided my 
eyes and muttered - `;
...
text(() => {
   ...
   Coco(T3C026)`"Yeah, all that running around's made me 
starved!"`;
   Pipi(PIPI_00)`"Warf!"`;
...
one who hadn't eaten yet.`;
   `I handed Coco the last chicken sandwich I had 
...
text(() => {
   ...
   Coco(T3C030)`"Mmm. This blend of soy sauce, mayonnaise and 
...
text(() => {
   ...
   `Coco shot the question to the Kid, who was 
...
text(() => {
   ...
   `Coco pulled out a tube she had hidden behind 
...
text(() => {
   ...
   Coco(T3C039)`"It's supposed to be for salads. It was the 
...
text(() => {
   ...
   Pipi(PIPI_00)`"Woof!"`;
   `With that, both Coco and the Kid covered their 
...
showTextbox();
text(() => {
   You(T3C050)`"Tsugumi's still asleep. She won't wake up for 
awhile, so..."`;
...
text(() => {
   ...
   You(T3C052)`"Yep, I'll bet she's pretty tired. Let's let 
...
showTextbox();
text(() => {
   Coco(T3C054)`"Okay then. So she can eat it later, after 
she wakes up."`;
   Takeshi`"Huh?"`;
...
showTextbox();
text(() => {
   Coco(T3C055)`"Let's save it for her. No snacking on it on 
either!"`;
   `Coco pointed to the sandwich nestled deep in 
the food warmer.`;
   `...So she'd noticed it.`;
...
text(() => {
   ...
   `Coco, the Kid and Pipi ran toward the rest 
area together.`;
...
text(() => {
   ...
   You(T3C059)`"Yeah, actually, I just stopped by the 
...
showTextbox();
text(() => {
   You(T3C060)`"I'm not a medical expert or anything, so I 
can't say anything specific."`;
   You(T3C061)`"But she seems to be recuperating. 
...
text(() => {
   ...
   You`"..."`;
   `You suddenly stopped talking and let her head 
fall forward.`;
...
text(() => {
   ...
   Takeshi`"??"`;
   `You lifted her head as if she had just 
realized something.`;
...
showTextbox();
text(() => {
   You(T3C067)`"Perhaps I've lost some of my confidence, but 
...
showTextbox();
text(() => {
   `I wondered if there were some kind of meaning 
to all of it.`;
   `I asked You, but she just shook her head 
unknowingly. `;
   `It looked like Coco, the Kid and Pipi were 
...
Sora was here until just before we ate."`;
   ...
   You(T3C072)`"It only happens once in a while, but 
sometimes she'll dive into LeMMIH's main 
console and not come out for quite a while."`;
   Takeshi`"Really?"`;
   You(T3C073)`"Seems like she can get pretty caught up in 
her work..."`;
...
the corridor.`;
   `As for me, after a quick stretch I let my 
eyes follow Coco and the others.`;
   `The same two people and one critter were 
circling about the stone statues.`;
   `It seemed they were playing a game of 
...
text(() => {
   ...
   `After her unexplained disappearance, Sora was 
...
flowers. `;
   Sora(T3C079)`"A bud sprouts from a seed. A flower blossoms, 
reproduces, a seed matures and returns to the 
earth."`;
   Sora(T3C080)`"From that seed comes another bud, another 
...
seed which in turn returns to the earth."`;
   ...
   Sora(T3C084)`"Whatever the reason or purpose, I think life 
is a wonderful thing."`;
   Sora(T3C085)`"When I see a flower struggling to exist, 
growing from a crack in the concrete, I have 
a strange and wonderful feeling."`;
   Sora(T3C086)`"Flowers are always striving desperately to 
live."`;
   Sora(T3C087)`"Even in an environment such as this, in the 
depths of the ocean, without enough light or 
carbon dioxide..."`;
   Sora(T3C088)`"Even in such parched and cracked soil..."`;
   Sora(T3C089)`"They never even think of giving up, but just 
struggle courageously to live."`;
   Sora(T3C090)`"Doesn't it inspire you?"`;
   Sora(T3C091)`"...Or, maybe not."`;
   Sora(T3C092)`"Maybe it's no use contemplating the meaning 
...
showTextbox();
text(() => {
   `The ceiling was designed so the water sprayed 
...
from the ceiling.`;
   You(T3C103)`"Oh man, we ran over here as soon as we saw 
...
showTextbox();
text(() => {
   `When I said that, You tossed the fire 
extinguisher aside.`;
...
direction.`;
   ...
   You(T3C110)`"Just who broke the group rules the other 
day, huh?"`;
