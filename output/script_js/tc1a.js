showTextbox();
text(() => {
   Girl(C1T000)`"It doesn't do us any good just standing 
around here, how about you take us somewhere?"`;
   //Park staff
   Park_staff(C1T001)`"Certainly. There is a relatively safe place 
...
we were in Zweite stock—basement level two.`;
   `A short time later...`;
   `After walking a few dozen feet a strange sound 
caught my attention.`;
...
text(() => {
   ...
   //Park staff
   Park_staff(C1T006)`"Due to all of the water pressure here at 
...
text(() => {
   ...
   `Maybe it was just like Sora said, and LeMU's 
...
showTextbox();
text(() => {
   `The three of us, myself and the other two 
...
text(() => {
   ...
   `The voice that had answered seemed to be in 
good spirits.`;
   `Whoever it was didn't sound like they were 
weak, or injured.`;
...
the roof of the trapped elevator.`;
   `It was a little more than six feet, from where 
I was standing`;
   `Next, I....`;
...
looked like four rails attached to their side.`;
   `It seemed that the elevator used these rails 
to move up and down.`;
...
kind of heavy partition.`;
   ...
   `A section of the pipe had been closed off to 
...
Coco.... Try not to mix them up, okay?"`;
   `Pipi and Coco...`;
   `That's right...`;
   `This was the dog that had clamped onto that 
...
been trapped in another elevator...`;
   ...
   `I said that and got down on my stomach 
...
text(() => {
   ...
   `Coco jumped over and over, reaching out for my 
hand.`;
...
I looked around the elevator.`;
   `I realized that nobody was in there but Coco. 
...
showTextbox();
text(() => {
   Coco(C1T037)`"Aaii! Haiya! Awwww, just a little bit 
further..."`;
...
two voices coming from this elevator..."`;
   Coco(C1T042)`"Two?"`;
   Coco(C1T043)`"This whole time it's only been Pipi and me in 
here."`;
...
text(() => {
   ...
   `Coco looked back and forth between the faces 
...
showTextbox();
text(() => {
   Coco(C1T054)`"Aaah! You're that lady who was giving 
...
text(() => {
   ...
   Sora(C1T060)`"Because you were so enthusiastic when you 
...
text(() => {
   ...
   Sora(C1T064)`"I also remember your dog's name. Pipi, wasn't 
it?"`;
   Pipi(PIPI_01)`"Woof!"`;
...
text(() => {
   ...
   `Shaking his tail excitedly, for some reason 
...
text(() => {
   Coco(C1T067)`"Huh?"`;
   Coco(C1T068)`"Um, wait a second...you wouldn't happen to 
...
showTextbox();
text(() => {
   Girl(C1T072)`"Whatever. I don't really care about that, but 
...
text(() => {
   ...
   `Coco scooped up Pipi, and rapped him lightly 
on the head.`;
...
showTextbox();
text(() => {
   Coco(C1T077)`"By the way...I've got something I want to 
ask..."`;
   Sora(C1T078)`"?"`;
   Girl(C1T079)`"?"`;
   Takeshi`"?"`;
...
showTextbox();
text(() => {
   `While we were walking, Sora gave Coco a brief 
...
to here.`;
   ...
   Coco(C1T082)`"You mean, water from the ocean came gushing 
into LeMU!?"`;
   `Taken off guard, Coco looked as if she'd been 
gut-punched.`;
...
the situation through to the next step.`;
   `From time immemorial, only people with these 
...
moment, it was simply how I felt.`;
   `Men protect women—`;
   `I was able to keep rational precisely because 
...
showTextbox();
text(() => {
   `While I was thinking about these trivial 
...
text(() => {
   ...
   //Park staff
   Park_staff(T1B006)`"For a while I wondered what 
...
text(() => {
   ...
   //Park staff
   Park_staff(T1B010)`"Seems like you had a rough 
time of it, too..."`;
...
text(() => {
   Kid(T1B013)`"......"`;
   `It was that kid who we'd taken to the 
infirmary.`;
   `His expression looked darker somehow.`;
   `If he'd become super upbeat all of the sudden 
...
text(() => {
   ...
   //Park staff
   Park_staff(T1B016)`"Hey, Sora, do you have any idea what our 
situation is?"`;
   `The girl asked Sora.`;
