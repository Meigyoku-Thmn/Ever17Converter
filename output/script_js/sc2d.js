...
text(() => {
   ...
   Sora(C2S219)`"That action allows me to manifest myself for 
the first time."`;
...
showTextbox();
text(() => {
   Sora(C2S225)`"Is there anything else? Please don't stare at 
me like that."`;
   `Sora was blushing.`;
...
showTextbox();
text(() => {
   Sora(C2S229)`"Well, now let's say that you have seen a full 
moon."`;

});
showTextbox();
text(() => {
   Sora(C2S230)`"And let's say that you closed your eyes. 
...
showTextbox();
text(() => {
   Sora(C2S231)`"And how about if everyone in the world closed 
...
showTextbox();
text(() => {
   Sora(C2S234)`"That goes against the purpose of this 
exercise."`;
...
showTextbox();
text(() => {
   Sora(C2S235)`"My question was whether there is anyway of 
measuring whether or not the moon exists, if 
someone closes their eyes."`;
   Kid`"?"`;
   Sora(C2S236)`"When you close your eyes, do you think 
...
showTextbox();
text(() => {
   Sora(C2S237)`"What if I told you that not only myself, but 
all of LeMU was nothing but an RSD ?generated 
illusion.....what would you do then?"`;
   Kid`"...Huh?"`;
   Sora(C2S238)`"Can you prove that the walls, the ceiling and 
...
text(() => {
   ...
   Sora(C2S241)`"You sounded so full of confidence when you 
...
text(() => {
   ...
   Sora(C2S243)`"For humans to know the world around them they 
must use one of their five senses."`;
   Sora(C2S244)`"If they lose all of these, they lose the 
ability to tell whether this world exists, 
or not."`;
   Sora(C2S245)`"Another way of putting it might be to say 
that the perspective is in the eye of the 
beholder."`;
   Sora(C2S246)`"In this instance, I don't mean just sight, 
...
showTextbox();
text(() => {
   Sora(C2S247)`"The point at which we perceive the world ? 
that is perspective."`;
   Sora(C2S248)`"And I..."`;
