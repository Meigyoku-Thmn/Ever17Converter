...
Takeshi, You, and the young Kaburaki.`;
   `They weren't able to give Coco a shot, 
...
effect.`;
   `The three infected 'patients' were lying, 
...
text(() => {
   `——Suddenly!`;
   `From his barely conscious state, a noise from 
the speaker reached Takeshi's ears.`;
   ...
   //Mission Control
   Mission_Control(T6T224)`"This is the emergency rescue team from the 
...
showTextbox();
text(() => {
   Takeshi(C6A295)`"Yeah! We can hear you! We hear you! Loud and 
clear!"`;
...
where they are located?"`;
   Takeshi(C6A297)`"In the examination room there are five...no, 
six."`;
   //Mission Control
...
showTextbox();
text(() => {
   Takeshi(C6A299)`"S-something goes wrong...? W-what do you 
mean...?"`;
   //Mission Control
   Mission_Control(T6T235)`"The partition strength threshold is already 
being exceeded."`;
   //Mission Control
...
get to you before that happens."`;
   //Mission Control
   Mission_Control(T6T239)`"We're heading there, no matter what! So, 
...
showTextbox();
text(() => {
   Takeshi(C6A302)`"Damn it, we can't open up communications 
from this side..."`;
   `Takeshi began to pour through the manual on 
...
diving.`;
   `The important thing was, it could be 
brought over to the pool by remote control.`;

});
showTextbox();
text(() => {
   Takeshi(C6A305)`"If things get dangerous, I guess they want 
us to board this."`;
...
showTextbox();
text(() => {
   `Suddenly a low metallic crashing sound 
...
showTextbox();
text(() => {
   Sora(T6T247)`"There is over six inches of flooding 
...
text(() => {
   ...
   `He crawled and stood, fell again and rose 
again...then he fell backward....`;
   `Repeating this process over and over, he 
...
showTextbox();
text(() => {
   Takeshi(C6A336)`"Don't you stop me, you hear? No matter what 
you say, I'm going."`;
...
showTextbox();
text(() => {
   `Arriving at Zweite stock, he threw himself 
into the water.`;
   `In order to reach the corridor connecting to 
the infirmary... Takeshi dove.`;
   `At this point, there was no way to access it 
without swimming.`;
   fadeClearPage();
});
hideTextbox();
...
showTextbox();
text(() => {
   `*SPLASH!*—Takeshi sucked in air greedily 
...
showTextbox();
text(() => {
   Takeshi(C6A343)`"I came back for something I forget, um, yeah 
that's it."`;
...
slowly.`;
   ...
   Takeshi(C6A346)`"More important to me than anything 
else...friend..."`;
...
text(() => {
   ...
   Takeshi(C6A348)`"Actually, in a 25 meter pool...I made it 
down and back, once..."`;
...
showTextbox();
text(() => {
   Takeshi(C6A350)`"Wait, I think I just beat my record a while 
a go..."`;

});
showTextbox();
text(() => {
   Takeshi(C6A351)`"Hey? Do you remember the furthest I've ever 
swam underwater?"`;
...
break down.`;
   ...
   Takeshi(C6A355)`"I still haven't gotten enough abuse from you 
yet."`;
   Takeshi(C6A356)`"I want to hear what you've got to say, from 
here on out..."`;
...
the flustered Sora off with his hands.`;
   ...
   Takeshi(C6A362)`"Relax.... I still have one more thing I've 
forgotten..."`;
...
could be heard...`;
   You(C6A366)`"I was able to finish the transfer just in 
time."`;
...
showTextbox();
text(() => {
   `And letting out a strange shout, Takeshi 
...
text(() => {
   ...
   `It was a single terabyte disk made out of 
polycarbonate.`;
...
text(() => {
   ...
   `Tsugumi slumped over in her chair, breathing 
with her whole body.`;
...
text(() => {
   ...
   Sora(C6A386)`"There will be a time when we meet again. I'm 
sure of it."`;
...
text(() => {
   ...
   Takeshi(C6A393)`"That the only place to find you is here, yet 
...
text(() => {
   ...
   Takeshi(C6A396)`"When you hit a button, don't forget to say 
'Keep it simple'. Remember that."`;
   Sora(C6A397)`"Yeah...heh, heh."`;
   Sora(C6A398)`"Don't worry. I will remember."`;
   Sora(C6A400)`"That's a promise. Mine and yours, 
professor..."`;
...
IBF.`;
   `On the surface of the pool, there was not 
even a ripple.`;
...
text(() => {
   ...
   Takeshi(C6A406)`"Yeah, they called themselves the Maritime 
Defense Force, I think."`;
   Takeshi(C6A407)`"You was able to open up the communications 
...
showTextbox();
text(() => {
   `And talking this way, they made their way to 
...
text(() => {
   ...
   Takeshi(C6A408)`"Yeah..."`;
   Takeshi(C6A409)`"You, the Kid, Coco, that old guy...they're 
all gone."`;
...
showTextbox();
text(() => {
   Takeshi(C6A410)`"Hey, hold on. I'm sure the rescue team found 
them."`;
   `Just then...`;
...
text(() => {
   ...
   //Mission Control
   Mission_Control(C6A424)`"Roger. We'll send the team back there as 
soon as we can."`;
   //Mission Control
   Mission_Control(C6A425)`"But there is the possibility that the team 
...
showTextbox();
text(() => {
   Takeshi(C6A426)`"I've got the exploration vessel standing by. 
I can call it here by remote control, right?"`;
   //Mission Control
   Mission_Control(C6A427)`"Ah, yeah...I'm sorry to have to ask you to 
use it, but there's nothing else we can do."`;
   //Mission Control
   Mission_Control(C6A428)`"We'll do the best we can. Whatever happens, 
you hang in there."`;
...
text(() => {
   ...
   Takeshi(C6A432)`"All this would be for nothing, if everyone 
died while they were waiting for us."`;
   Takeshi(C6A433)`"The rescue team decided to put a higher 
...
text(() => {
   Takeshi(C6A434)`"Well..."`;
   Takeshi(C6A435)`"I might just take a nap until the next bus 
comes..."`;
...
showTextbox();
text(() => {
   Takeshi(C6A436)`"If you think the bus is going to be late, 
call a cab for me, will you?"`;
   Tsugumi(T6T316)`"T-taxi?"`;
   Tsugumi(T6T317)`"What are you talking about...Takeshi?"`;
   Takeshi(C6A437)`"If you push that button, a small submarine 
...
text(() => {
   ...
   `That's right...I injured my hand in the Whale 
Room...and then...`;
...
tell it was an alarm ringing somewhere far off.`;
   ...
   `Inside my head was still dim, and it was hard 
...
text(() => {
   You(S6A089)`"Kid!"`;
   `You jumped into the room, as though she were 
possessed by a demon.`;
   You(S6A091)`"I found an exit we can escape through!"`;
   `You was screaming.`;
   `'I found an exit'...I said these words to 
...
showTextbox();
text(() => {
   You(S6A093)`"Takeshi's going to get Mayo, and Tsugumi's 
...
text(() => {
   ...
   `For no reason in particular I repeated You's 
words.`;
   `Maybe she finally noticed my odd state. You 
...
text(() => {
   ...
   `Kaburaki, Tsugumi and Sara were already there 
waiting.`;
...
text(() => {
   ...
   `Kaburaki grabbed my shoulder hard and, turning 
...
text(() => {
   ...
   You(S6A097)`"We were finally able to make contact with the 
surface and access the emergency stairs."`;
   You(S6A098)`"But if we don't hurry, they're going to 
flood."`;
   You(S6A099)`"So hurry."`;
...
showTextbox();
text(() => {
   `And tugging on it violently, You started 
...
keep pace with her as she pulled me forward.`;
   `We flew liked the wind down the painfully 
cramped passage...`;
...
of the metal bulkhead.`;
   `You's eyes met mine and nodding to one 
...
have been solved...)`;
   `And as the doubts went through my mind, my 
legs faltered.`;
...
out of here...?)`;
   `And as this thought filled my mind, I had 
...
text(() => {
   `*Clank* `;
   `The door unlocked.`;
   `And bracing my chest and shoulder against the 
...
poured down gently on our heads from above.`;
   `All of us, me, You, Sara, Tsugumi and 
