...
text(() => {
   ...
   `I could find nothing by which to judge 
distance.${wait(30)}`;
   `I searched for light, but couldn't even see 
...
text(() => {
   ...
   `It seemed to me, rather, that the darkness was 
coming toward me.${wait(30)}`;
...
nightmare.`;
   ...
   `Coco and Pipi were nestled together 
comfortably, asleep.`;
...
text(() => {
   ...
   `For a moment, I had the illusion it went on 
forever.`;
   `(Stop being stupid...)`;
...
text(() => {
   ...
   `I wondered if Tsugumi had cleaned them up, 
...
text(() => {
   `(Well, how about that?)`;
   `(Geez, Sora. I guess you forgot to tell me 
about this.)`;
   `(Or then again maybe I just wasn't 
listening...)`;
...
and then...`;
   `Certainly not expecting to be throwing it up 
...
showed no sign of budging.`;
   `But of course.`;
   `All of the elevators in this section of the 
...
text(() => {
   ...
   Tsugumi(T5A002)`"There's no point waiting around. That 
...
text(() => {
   ...
   Tsugumi(T5A015)`"And it's located just behind this elevator 
room."`;
...
text(() => {
   ...
   Tsugumi(T5A017)`"Well, Qualle is just behind this elevator 
room."`;
...
showTextbox();
text(() => {
   Tsugumi(T5A019)`"It's already turned on, dummy, just get in 
and it'll go."`;
   Takeshi`"That's it?"`;
   Tsugumi(T5A020)`"Look, I tried it out already, so I'm pretty 
sure."`;
...
showTextbox();
text(() => {
   `And then suddenly, her expression hardened 
again.`;
...
text(() => {
   ...
   Tsugumi(T5A042)`"Things to say...things I need to let you 
know..."`;
   Tsugumi(T5A043)`"And I figured, no one could interrupt us 
here."`;
...
now is..."`;
   ...
   Tsugumi(T5A058)`"You see...I was born 24 years ago. In July 
1993."`;
...
showTextbox();
text(() => {
   Tsugumi(T5A060)`"There's something I've been meaning to ask 
you."`;
   Takeshi`"What is it...?"`;
   Tsugumi(T5A061)`"Well, it's something I wanted to 
...
wet and smooth.`;
   ...
   Tsugumi(T5A071)`"Well then, is the nail that's been cut away 
still you?"`;
...
it's not me anymore."`;
   ...
   Tsugumi(T5A074)`"In that case, this hair here is you right? 
...
showTextbox();
text(() => {
   `Me...Takeshi Kuranari. Since the moment I was 
born I had existed as one person. One 
continuous, linear, cohesive individual.${wait(30)}`;
...
no longer 'Kuranari Takeshi?'${wait(30)}`;
   `Thoughts? Senses? Emotions? Sensations? 
Character? Values?${wait(30)}`;
...
molecules than the 'me' there.${wait(30)}`;
   `From purely a material point of view, my 
...
showTextbox();
text(() => {
   `They say the cells in the entire human body 
...
'me' then were definitely the same.${wait(30)}`;
...
breathed easier.`;
   ...
   Takeshi`"A concept?"`;
   Tsugumi(T5A085)`"You are just a packet of 
information...'software'."`;
   Tsugumi(T5A086)`"The information written on a CD has no 
physical body, right?"`;
   Tsugumi(T5A087)`"The CD itself is just plastic, a hard mass of 
polycarbonate."`;
   Tsugumi(T5A088)`"But that mass has nothing to do with the 
information it holds."`;
   Tsugumi(T5A089)`"Information can't have a physical body, 
that's impossible."`;
   Tsugumi(T5A090)`"Information does not have shape or 
substance."`;
   Tsugumi(T5A091)`"The embodiment of information only takes 
place through its application."`;
   Tsugumi(T5A092)`"However, the embodied information itself has 
no actual body."`;
   Tsugumi(T5A093)`"The essence of information is the information 
itself..."`;
   Takeshi`"........."`;
   Tsugumi(T5A094)`"Takeshi, it's the same for you."`;
   Tsugumi(T5A095)`"There is no physical substance to your 
'essence.'"`;
   Tsugumi(T5A096)`"Instead, the human called 'Takeshi Kuranari' 
is no more than a concept, information, 
software."`;
   Tsugumi(T5A097)`"And that essence is realized only through 
your body, the hardware."`;
   Tsugumi(T5A098)`"That's it. Your body is just the hardware."`;
   Tsugumi(T5A099)`"It's just a device in which the entity of 
'you' is embodied."`;
   Takeshi`"...I see. Hmm."`;
...
talking about?"`;
   Tsugumi(T5A100)`"Twelve years ago, I was infected with a 
virus."`;
   ...
   Tsugumi(T5A105)`"Normally, the human body is totally replaced 
with new cells within five years."`;
   Tsugumi(T5A106)`"When I was twelve, the cells in my body were 
infected. And from that moment on, they 
divided and replaced themselves."`;
   Tsugumi(T5A107)`"Five years later, all cells had been 
replaced."`;
   Tsugumi(T5A108)`"In five years, every cell that made up 
my body had been genetically altered."`;
   Takeshi`".........?"`;
   Tsugumi(T5A109)`"And when the last of the cells had died..."`;
   Tsugumi(T5A110)`"...the last trace of my 12-year-old self 
disappeared."`;
   Tsugumi(T5A111)`"In that moment the old me died."`;
   Tsugumi(T5A112)`"The old me died, but the new me became a body 
that never dies."`;
   Takeshi`"A body that never dies...?"`;
   Tsugumi(T5A113)`"My immune system and my metabolic 
...
themselves..."`;
   ...
   Tsugumi(T5A117)`"My physical growth stopped at that 
moment in time."`;
...
dying - was it possible?`;
   `Why do living creatures age? ...Essentially, 
it is to protect against exhaustion and 
deterioration of function.`;
   `The life cycle needs aging so it can have 
...
solid."`;
   ...
   Tsugumi(T5A121)`"I thought you might fall for it, but I guess 
not."`;
...
her legs.`;
   ...
   `However...`;
   `It looked like something that had healed years 
...
showTextbox();
text(() => {
   `All this time, Tsugumi continued to hold my 
neck with her hands.`;
...
text(() => {
   ...
   Takeshi`".........?"`;
   Tsugumi(T5A130)`"You are just a packet of 
information...'software'."`;
   Tsugumi(T5A131)`"The information written on a CD has no 
physical body, right?"`;
   Tsugumi(T5A132)`"The CD itself is just plastic, a hard mass of 
polycarbonate."`;
   Tsugumi(T5A133)`"But that mass has nothing to do with the 
information it holds."`;
   Tsugumi(T5A134)`"Information can't have a physical body, 
that's impossible."`;
   Tsugumi(T5A135)`"Information does not have shape or 
substance."`;
   Tsugumi(T5A136)`"The embodiment of information only takes 
place through it application."`;
   Tsugumi(T5A137)`"However, the embodied information itself has 
no actual body."`;
   Tsugumi(T5A138)`"The essence of information is the information 
itself..."`;
   Takeshi`"..."`;
   Tsugumi(T5A139)`"Takeshi, it's the same for you."`;
   Tsugumi(T5A140)`"There is no physical substance to the essence 
of Takeshi."`;
   Tsugumi(T5A141)`"Instead, the human called 'Takeshi Kuranari' 
is no more than a concept, information, 
software."`;
   Tsugumi(T5A142)`"And that essence is realized only through 
your body, the hardware."`;
   Tsugumi(T5A143)`"That's it. Your body is just the hardware."`;
   Tsugumi(T5A144)`"It's just a device in which the entity of you 
is embodied."`;
   Takeshi`"Why...do...you...?"`;
...
showTextbox();
text(() => {
   Tsugumi(T5A168)`"I'll give you the gift of death! And then 
I'll..."`;
   Takeshi`"Argh......!!"`;
...
up inside me.`;
   ...
   `And I plunged into the chaos, further and 
...
showTextbox();
text(() => {
   `Afterwards, I went straight back to the 
Conference Room.`;
...
herself in the Conference Room.`;
   You(T5A181)`"So how about it Sora? How did the maintenance 
go?"`;
...
text(() => {
   ...
   Sora(T5A183)`"The system is in sound condition. Although, 
there would be no way to repair any mechanical 
damage if it were to occur."`;
   Sora(T5A184)`"The scheduled maintenance program checked 
...
showTextbox();
text(() => {
   Sora(T5A186)`"After carefully assessing the situation, I 
have found a slight noise in my thought 
processor..."`;
   Sora(T5A187)`"Normal function will not be compromised, so 
...
text(() => {
   ...
   Sora(T5A190)`"You see, sometimes certain customers want to 
play pranks on me. They tell lies, or try to 
confuse me."`;
   Sora(T5A191)`"And because of that, my memories or 
programmed responses get a little out of 
alignment. It tends to cause issues in my 
thought processing."`;
   Sora(T5A192)`"Sometimes older data gets mixed with newer 
information, making it difficult to determine 
which is correct."`;
   Sora(T5A193)`"When this problem becomes severe, it's 
...
showTextbox();
text(() => {
   Sora(T5A194)`"However, at this time, nothing too drastic 
...
showTextbox();
text(() => {
   Sora(T5A203)`"Alright everyone, why don't we go to 
breakfast?"`;
   `"W...What?"`;
   `(Is it me, or did she just totally ignore my 
question?)`;
   `...Oh well...`;
...
showTextbox();
text(() => {
   `Everybody relocated to the chicken sandwich 
shop.`;
...
felt relieved.`;
   `The faces of You, Coco, and the Kid seemed so 
happy...`;
