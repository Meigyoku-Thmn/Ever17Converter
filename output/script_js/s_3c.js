...
text(() => {
   ...
   Sara(S3B004)`"But didn't you think that it might make 
Takeshi angry?"`;
...
text(() => {
   ...
   Tsugumi(S3B007)`"Maybe I did want to play a trick on him just 
a little bit."`;
   Kid`"?"`;
   Sara(S3B008)`"?"`;
...
showTextbox();
text(() => {
   Tsugumi(S3B009)`"Hee, hee...he's even more stubborn than I 
thought."`;
...
text(() => {
   ...
   Takeshi(S3B011)`"Why do you think something would go wrong all 
of a sudden?"`;
...
showTextbox();
text(() => {
   Tsugumi(S3B013)`"It's probably because you were all running 
...
text(() => {
   Everyone`"........."`;
   `Tsugumi's flippant remark had tensed the 
...
showTextbox();
text(() => {
   Tsugumi(S3B017)`"What's wrong? If you don't hurry I'll leave 
you behind."`;
...
text(() => {
   ...
   `For a while all we could do is stand around 
...
showTextbox();
text(() => {
   Sora(S3B022)`"I will run a scan to determine the situation 
inside."`;
...
showTextbox();
text(() => {
   Takeshi(S3B023)`"The steam in there isn't going to explode, if 
...
showTextbox();
text(() => {
   Sora(S3B029)`"Atmospheric pressure is normal. No traces of 
toxic gas detected, however, the temperature 
and humidity of the room are slightly 
elevated."`;
   Sora(S3B030)`"An area of the pipe has ruptured, and there 
...
showTextbox();
text(() => {
   Tsugumi(S3B033)`"Nothing. If that's all that's wrong, it 
...
showTextbox();
text(() => {
   Takeshi(S3B035)`"Let's go in. If we fix it, there won't be a 
problem, right?"`;
...
text(() => {
   `We opened the door.`;
   `We looked toward the wall that was making the 
hissing sound.`;
...
text(() => {
   ...
   Tsugumi(S3B039)`"Sure. Something small like this we can fix in 
a second."`;
...
text(() => {
   ...
   Tsugumi(S3B041)`"The pipe bit bent...I'm going to reset the 
...
showTextbox();
text(() => {
   Tsugumi(S3B044)`"Over there...there is a valve connecting to 
...
showTextbox();
text(() => {
   Tsugumi(S3B046)`"I'll tell you which tools I need and you pass 
them to me."`;
   Kid`"Got it."`;
...
text(() => {
   ...
   Tsugumi(S3B052)`"You know, asking meaningless questions is a 
bad habit of yours."`;
   Takeshi`"........."`;
...
words seemed out of place.`;
   `I stole glances at Tsugumi's expression as I 
...
text(() => {
   ...
   Sora(S3B057)`"It's from You. It appears that she is in the 
Security Office."`;
...
showTextbox();
text(() => {
   Sora(S3B059)`"Excuse me everyone, but You is calling me. I 
must be going."`;

});
showTextbox();
text(() => {
   Takeshi(S3B060)`"Okay, go for it. We should be able to handle 
the rest ourselves."`;
...
as if it had sucked her in.`;
   `We knew she could travel instantly, but...it 
...
showTextbox();
text(() => {
   Tsugumi(S3B062)`"Kid? I'm all set here, so please work on the 
valves for me."`;
...
Conference Room from the kiosk this morning.)`;
   ...
   Tsugumi(S3B067)`"Kid, do the valve in the order that I just 
told you."`;
   Kid`"..."`;
...
text(() => {
   Kid`"........."`;
   `I remember losing all power to stand and 
...
showTextbox();
text(() => {
   Takeshi(S3B073)`"What're you talking about...? You aren't 
making sense."`;
...
showTextbox();
text(() => {
   Tsugumi(S3B079)`"You don't get it? I'm saying you're in the 
way."`;
   Kid`"........."`;
   Takeshi(S3B080)`"Hey, that's a little harsh don't you think? I 
...
text(() => {
   ...
   Tsugumi(S3B082)`"Listen. At times like this, people like him 
are a liability."`;
...
showTextbox();
text(() => {
   Tsugumi(S3B084)`"You don't want to cause an accident here, do 
you?"`;
...
showTextbox();
text(() => {
   Tsugumi(S3B086)`"Now you know what I mean. Get outta the way. 
...
it bothered me to no end.`;
   `Yesterday in the Generator Room and then this. 
I wondered what was wrong with me.`;
   `I was still without my memory.`;
   `I couldn't even find any of the pieces.`;
   `(But maybe...)`;
   `I thought that maybe these crazy flashes that 
...
it.`;
   `Suddenly, I noticed that deeper in the corner 
...
slowly across the gray walls.`;
   `I looked at the moon and its shadows, and let 
...
${noWait}`;
   Append`              ${nextPage(0)}I'm sorry, I'm sorry -
${noWait}`;
...
${noWait}`;
   Append(S3B097)`   All of this is just the way you wanted it.${waitVoice}
${noWait}`;
   Append(S3B098)`     Right? You best fess up to all of it.${waitVoice}
${noWait}`;
   Append`            You knew from the start 
${noWait}`;
...
${noWait}`;
   Append(S3B101)`                 You ran away.${waitVoice}
${noWait}`;
...
${noWait}`;
   Append(S3B103)`               And you broke it.${waitVoice}
${noWait}`;
...
${noWait}`;
   Append`       You bought happiness for yourself. 
${noWait}`;
   `     You traded it for that girl's life... 
${noWait}`;
...
away...`;
   Kid`"Damn you!"`;
   `I took the chair and with all my strength, 
...
text(() => {
   Kid`"I'll kill you."`;
   `Saying this I threw my body into the steel 
door.`;
...
light.`;
   ...
   `My body felt heavy like lead and like it would 
sink into the floor.`;
...
text(() => {
   `It was You.`;
   `Deja vu...`;
   `No, this was something that I had actually 
experienced before.`;
   `Two days before, on the first day, when I woke 
...
showTextbox();
text(() => {
   You(S3B112)`"I just replaced her a little bit ago. She 
...
my thumb.`;
   `Just maybe, the scar held some kind of clue to 
...
text(() => {
   ...
   `You poured me a cup of freshly brewed coffee, 
...
text(() => {
   You(S3B114)`"I see..."`;
   You(S3B115)`"So that little room and the room that you 
were in before were similar somehow?"`;
   Kid`"Probably."`;
   You(S3B116)`"Listening to that it sounds like a sickroom 
...
showTextbox();
text(() => {
   You(S3B117)`"You've probably been hospitalized before 
somewhere."`;
   Kid`"..."`;
...
showTextbox();
text(() => {
   You(S3B120)`"I mean, maybe you've lost your memory or had 
violent fits repeatedly before."`;
   You(S3B121)`"So maybe you were detained in a sickroom with 
a tiny window like that..."`;
   `You touched on the harsh reality of the 
...
showTextbox();
text(() => {
   You(S3B122)`"Of course, it's not for sure or anything. 
It's just one possibility."`;
   You(S3B123)`"You probably had a waking dream... that is a 
possibility, too."`;
...
showTextbox();
text(() => {
   `Finally You gave me an easy to understand 
explanation about the connectedness of 
memories and dreams.`;
   `According to You, dreams and memory are 
...
animal that might appear in a dream.`;
   ...
   `I felt sure that it was probably just my mind 
...
showTextbox();
text(() => {
   You(S3B125)`"Okay. For now, shall we go to check it out 
one more time?"`;
   Kid`"Go check what?"`;
...
text(() => {
   ...
   You(S3B127)`"You might find a clue to help you jog your 
...
between that room and my memory.`;
   ...
   `If she were willing to take me, I couldn't 
turn her down.`;
...
text(() => {
   ...
   `I was amazed that I could have made this much 
...
text(() => {
   ...
   Kid`"Huh?"`;
   You(S3B130)`"If I remember correctly...you came into the 
...
text(() => {
   ...
   `You walked along the wall looking for the 
...
text(() => {
   ...
   You(S3B136)`"Geez. Can you believe they are still using 
electronic fossils like this these days?"`;
   `You muttered this as she stared at the light 
switch.`;
...
showTextbox();
text(() => {
   You(S3B137)`"They should spend money on places like this 
even if the customers won't see them."`;
   You(S3B138)`"It's because they cut corners in places like 
...
a warehouse.`;
   `I looked around for moonlight that had been 
...
showTextbox();
text(() => {
   `You followed my eyes and went and stood in 
...
showTextbox();
text(() => {
   You(S3B145)`"To change the AC power that comes from the 
generator into DC..."`;
   You(S3B146)`"It's a mechanism for changing the electrical 
current into one that the tools in this 
warehouse can use."`;
   You(S3B147)`"That means this device controls those 
functions."`;
   Kid`"I see..."`;
...
showTextbox();
text(() => {
   `You said it straight faced and without looking 
at me.`;
