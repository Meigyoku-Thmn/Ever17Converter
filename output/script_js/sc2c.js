...
text(() => {
   `After that...`;
   `We gathered at the rest area, had a light meal and chatted it up.`;
   `From beginning to end, Sora's explanation of the RSD system dominated the conversation.`;
   `During the talking, I discovered something of interest.`;
   `I learned that Sara was supposed to be amazing when it came to computers.`;
   `Sara was floored the day before when she heard that Sora was computer generated. But when Sora gave her a technical explanation, she was able to absorb it all without any problem.`;
   `You said this about Sara -`;

});
monoColorOverlay({ interval: 0, color: BLACK });
bgload({ name: `BG21B1`, transition: 26 });
fgload({ id: 1, name: `YU01ADL`, x: 320, useAnim: false });
...
text(() => {
   You(S2C081)`"I know."`;
   You(S2C082)`"When it comes to computers...she's just a genius."`;
...
text(() => {
   ...
   Sora(T2C231)`"At that instant, I would cease to exist anywhere."`;
...
showTextbox();
text(() => {
   `After the discussion, I stayed in the rest area alone.`;
   `It was a free time of sorts.`;
   `I imagined that everyone was walking around LeMU, spending the time as they pleased.`;
   `We had decided when and where we would meet.`;
   `I didn't know what I should do with myself until then.`;
   `I was bored stiff.`;
   `I looked down.`;
   ...
   `I reached into to the water trying to grab the light.`;
   `But...`;
   `I could only grab a little seawater...I couldn't touch the light.`;
   `I tried again and again, but no matter how I tried, I couldn't grab the moon.`;
   `The words that I had just heard from Sora cut across my mind again.`;
   Sora(T2C229)`"I mean, that I cease to exist when no one is looking at me."`;
...
text(() => {
   ...
   `As I thought, my consciousness slipped into a light sleep.`;
   `.......`;
   `....`;
   `...`;
...
showTextbox();
text(() => {
   Sora(S2C116)`"Kid, everyone is waiting for you."`;
   `Sora said it softly, peering down at me spread out on the bench.`;
   `I was still half a sleep when Sora and I started heading for the Conference Room to meet everyone.`;

});
bgload({ name: `BG25B3`, transition: 20 });
showTextbox();
text(() => {
   `I thought as I walked the corridors in a stupor.`;
   `I remembered the words that Sora had said before I slept - `;
   `"I mean, that I cease to exist when no one is looking at me."`;
   `For some reason, I just could not shake those words from my mind.`;
   `I asked Sora walking next to me why she had said them.`;
   `And this is what she said...`;

});
if (l_coco_point == 5) goto(lbl_0000031f);
l_choice = 1;
jump(`S_2C`);
let lbl_0000031f;
jump(`SC2D`);
