...
text(() => {
   ...
   Sora(C2T001)`"That action allows me to manifest myself for the first time."`;
...
text(() => {
   ...
   `No, it would be better to say that I couldn't say or do anything more.`;
   `It was hard for me to grasp the meaning of Sora's words.`;
...
showTextbox();
text(() => {
   `Going into the room, I could hear the beeping of the console, indicating someone was calling. `;
...
showTextbox();
text(() => {
   Sora(C2T007)`"Is there anything else? Please don't stare at me like that."`;
   `Sora was blushing.`;
...
text(() => {
   ...
   `But it was one that caused me to burst out laughing.`;
...
text(() => {
   ...
   `I didn't have anything in particular to do, so I wandered aimlessly down the corridors of Dritte stock.`;
   `Ahead of me I could see the door leading to the Lemurian Ruins. The door was open.`;
...
showTextbox();
text(() => {
   `I was curious, so I ducked inside the room with the ruins.`;
...
text(() => {
   `(Ah... who cares.)`;
   `Not paying it any more of my attention, I quickly passed by the entrance.`;

});
removeBG({ mode: BLACK, transition: 2 });
showTextbox();
text(() => {
   `And, letting my legs lead the way, I roamed without any destination in mind. Eventually I ended up resting on a bed in the infirmary staring out into the ocean depths...`;
...
showTextbox();
text(() => {
   `It turned out the figures in the room were You and Coco.`;
...
text(() => {
   ...
   `The two of them were flush up against one another, speaking in two and three word sentences. They looked like they were having fun.`;
   Pipi(PIPI_09)`"Wow! Wow! Woof!"`;
   `From out of nowhere Pipi appeared and rushed up to my legs.`;
   `I lifted up the sopping wet Pipi, and went over to stand by the two girls.`;
...
text(() => {
   ...
   Coco(C2T038)`"These are from when Nakkyu was a little bitty baby!"`;
...
showTextbox();
text(() => {
   `Come to think about it, I'd heard rumors of something like that before.`;
   `But I wouldn't have expected it to be anything more than a fleeting fad. There was no way I had any idea it was a deep-rooted girl tradition.`;
   `This album had probably been in with You's things from the changing room.`;

});
multifgload2({ id1: 1, id2: 2, name1: `YU03ADM`, name2: `CO11ADM`, x1: 128, x2: 512, useAnim: true });
showTextbox();
text(() => {
   You(C2T060)`You murmured softly to herself as she flipped through the pages.`;
   `I wonder what everyone is doing right about now...`;
...
text(() => {
   ...
   Coco(C2T065)`"So, I'll bet your mom and dad are worried about you?"`;
   Takeshi`"Well, they're parents. Parents worry, it's what they're good at..."`;
   Takeshi`"Buuut, that's only if they know we're stuck in here."`;
   Coco(C2T066)`"Well, they've got to know."`;
   Coco(C2T067)`"An accident as big as this? It's probably all over the news. They might even be doing a huge TV special about it."`;
...
text(() => {
   ...
   Takeshi`"I get a phone call every couple of months from home and I don't really call them much myself."`;
   Takeshi`"So....there's probably no way that my parents know that I'm trapped in a place like this."`;
...
text(() => {
   ...
   You(C2T069)`"An accident as big as this? It's probably all over the newspapers and television."`;
...
text(() => {
   ...
   `It didn't seem like she was very interested in what I had to say.`;
...
text(() => {
   ...
   You(C2T083)`"I don't know why, but....yaaawn....I'm kind of tired all of a sudden..."`;
...
showTextbox();
text(() => {
   `You and Coco looked over at me disapprovingly and, fighting off yawns...they both left the room.`;
   `Holding Pipi, I watched the two disappear from sight.`;
