...
public class CommandHandler {
	...
	private Map<String, Never7GraphicsState> gotoGraphicsStates;
	private Map<String, MusicState> gotoMusicStates;
	private List<String> choiceOpts;
			...
			FileExts exts)
	{
		...
		gotoGraphicsStates = new HashMap<String, Never7GraphicsState>();
		gotoMusicStates = new HashMap<String, MusicState>();
		choiceOpts = new ArrayList<String>();
	}
	...
	public String process(String line) {	
		...
		if (mode != null) {
			switch (mode) {
			case TEXT: return processText(line, scanner);
			}
		}
		...
		if (cmdOp != null) {
			...
			case fgload: {
				...
				gstate.setSprite(id, slot, filename);
			...
			case createPanBG: {
				...
				gstate.clearSprites();
				if (pos == 2 || pos == 3) {
					gstate.setBackground(gstate.toPanBGFilenameBottom(filename), -1);
				} else {
					gstate.setBackground(gstate.toPanBGFilenameTop(filename), -1);
				}
				return gstate.maybeFlush();
			}
			case movePanBG: {
				...
				String filename = gstate.getBackground().replaceAll("-(bottom|middle|top)\\.", ".");
				...
				gstate.setBackground(gstate.toPanBGFilenameMiddle(filename), fadeFrames);
				sb.append(gstate.maybeFlush());
				sb.append('\n');
				
				if (targetPos == 2 || targetPos == 3) {
					gstate.setBackground(gstate.toPanBGFilenameBottom(filename), fadeFrames);
				} else {
					gstate.setBackground(gstate.toPanBGFilenameTop(filename), fadeFrames);
				}
				sb.append(gstate.maybeFlush());
				sb.append('\n');
		...
		if (!cmd.equals("unsupported")) {
			if (unsupported.add(cmd)) {
				if (warnUnsupported) Log.v("Unsupported command: " + cmd);
			}
		}
		return "#" + line;			
	}
	...
	protected String processTextCommand(String line) {
		...
		if (op != null) {
			...
			default:
				if (warnUnsupported) Log.v("Unsupported str-command: " + cmd);
				return "#" + line;
			}
		} else {
			if (warnUnsupported) Log.v("Unknown str-command: " + cmd);
		}
		return "#" + line;
	}
	...
	protected void onLabel(String label) {
		...
		if (gstored != null) gstate = gstored;
		MusicState mstored = gotoMusicStates.get(label);
		if (mstored != null) mstate = mstored;
	}
