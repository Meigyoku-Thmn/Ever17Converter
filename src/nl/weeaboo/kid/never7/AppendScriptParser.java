...
public class AppendScriptParser extends WorkManager {
	...
	public AppendScriptParser(File srcF, File dstF, Dim srcSize, ResourcesUsed re, ImageInsets ii, FileExts fe) {
		...
		scriptHeaders = new ArrayList<ScriptHeader>();
	...
	public void convertFolder() throws IOException {
		...
		FileUtil.collectFiles(files, srcF, false, false, new FileCollectFilter() {
			...
			public boolean accept(String relpath, File file) {
				return file.isDirectory() || StringUtil.getExtension(relpath).equalsIgnoreCase("snr");
			}
		});
	...
	public void convertFile(WorkItem wi) throws IOException {
		...
		List<String> lines = new LinkedList<String>(Arrays.asList(readLines(wi.getFile(), encoding)));
		...
		try {
			...
			for (String line : readLines(decF, "UTF-8")) {
				...
				if (scanner.hasNext()) {
					...
					} else if (cmd.equals("jump")) { //Fix path for jump
	...
	private String toSpriteFilename(String filename, int timeOfDay) {
		...
		if (timeOfDay == 1) filename += "na"; //not sure
		if (timeOfDay == 2) filename += "n";
		return filename;		
	}
	...
	private static enum Command {
		...
		public static Command get(String name) {
			if (nameLookup == null) {
				nameLookup = new HashMap<String, Command>();
