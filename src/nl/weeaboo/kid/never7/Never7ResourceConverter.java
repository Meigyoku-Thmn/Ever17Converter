...
public class Never7ResourceConverter extends KIDResourceConverter {
	...
	protected TrackFile findTrackFile(File srcF) throws IOException {
		...
		File[] files = srcF.listFiles(new FilenameFilter() {
			@Override
		...
		if (files != null && files.length >= 1) {
			Arrays.sort(files, new Comparator<File>() {
				...
				public int compare(File fa, File fb) {
					...
					if (a.equals("mute")) return 1;
					if (b.equals("mute")) return -1;
					
					//Recommended .track file by translation patch
					if (a.equals("psp-ost-95.track")) return -1;
					if (b.equals("psp-ost-95.track")) return 1;
