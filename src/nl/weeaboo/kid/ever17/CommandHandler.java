...
public class CommandHandler {
	...
	private Map<String, GraphicsState> gotoGraphicsStates;
			...
			FileExts exts)
	{
		...
		gotoGraphicsStates = new HashMap<String, GraphicsState>();
	...
	public String process(String line) {	
		...
		if (mode != null) {
			...
			case SWITCH: return processSwitch(line, scanner);
			}
		}
	...
	protected String processSwitch(String line, Scanner scanner) {
		...
		String cmd = scanner.next();
		if (cmd.equals("switch_varop")) {
			scanner.skip("\\s*\\(.*?\\)");
			switchVarname = KIDUtil.getVarname(scanner.next());
			return "";
		} else if (cmd.equals("27")) {
			scanner.skip("\\s*->");
